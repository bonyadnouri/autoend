...
export function ExportMenu({ test, investigation }: Props) {
  ...
  const splitBtn =
    "inline-flex items-center justify-center gap-2 bg-brand-600 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 focus-visible:ring-offset-2";

  return (
    <div className="relative" ref={ref}>
      <div className="inline-flex rounded-lg">
        <button type="button" className={`${splitBtn} rounded-l-lg px-4`} onClick={createJira}>
          <Ticket size={16} /> Create Jira ticket
        </button>
        <button
          type="button"
          className={`${splitBtn} rounded-r-lg border-l border-brand-500/40 px-2.5`}
          onClick={() => setOpen((o) => !o)}
