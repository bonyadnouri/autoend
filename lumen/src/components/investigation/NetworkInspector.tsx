...
function statusTone(status: number): string {
  // 0 = no response (aborted/cancelled request) — neutral, not pass or fail.
            ...
            return (
              <Fragment key={req.id}>
                ...
                >
                  ...
                  <td className="break-all px-4 py-2.5 font-mono text-xs text-slate-700">
                    {req.endpoint}
                  </td>
                  <td className={`px-4 py-2.5 font-semibold ${statusTone(req.status)}`}>
                    {req.status === 0 ? "aborted" : req.status}
                  </td>
