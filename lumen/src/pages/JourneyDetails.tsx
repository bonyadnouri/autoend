...
export function JourneyDetails() {
  const { id } = useParams();
  const { getJourney, getScreen, getTest } = useAnalysisData();
            ...
            {journey.steps.map((step, i) => {
              ...
              return (
                <li key={i} className="relative flex gap-4 pb-6 last:pb-0">
                  ...
                  <Link
                    to={screenHref(step.screenId)}
                    className="flex-1 rounded-lg border border-slate-100 px-3 py-2.5 hover:border-slate-200 hover:bg-slate-50"
                  >
