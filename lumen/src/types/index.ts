export type TestStatus = "pass" | "fail" | "warning" | "not-executed" | "running";
export type JourneyStatus = "healthy" | "warning" | "broken";
export type ScreenRunStatus = "running" | "passed" | "failed" | "discovered";
export type ScreenStatus = JourneyStatus | ScreenRunStatus;
...
export interface TestScenario {
  ...
  /** Numbered, human-readable reproduction recipe (from the run timeline). */
  reproSteps?: string[];
  /** Exact executable Playwright flow autoend replays — the concrete reproduction. */
  script?: string | null;
}
...
export interface EvidenceShot {
  ...
  caption: string;
  tMs: number;
  /** When set, a real screenshot is rendered instead of the placeholder. */
...
export interface Insight {
  ...
  issueId?: string;
  /** Actionable next step the run suggests for this observation. */
  suggestedFix?: string;
}
