...
- writes real results into `tests` (from flows), `issues` (from findings), `journeys` (one per verified flow), `insights` (from findings, each with a suggested fix), and `investigations` (evidence detail) under the `shopflow-default` analysis, and updates that analysis's summary counts;
- builds the **application map** — every screen the agents reached and the navigation edges between them — and links each screen to the tests and issues that touched it, so the map is generated by the Run rather than seeded;
- surfaces pages the AI *expected* but that aren't really there (e.g. a `/signup` that 404s) as amber **warning** nodes, while a broken link you actually clicked stays a red failure;
...
When the variables are unset, the Run still completes and writes its local artifact; it just prints a warning and skips publishing.

### Live dashboard & running from the UI

Beyond one-shot CLI runs, autoend can run as a **daemon** that drives the [Lumen dashboard](https://github.com/an2323/lumen) end to end:

```sh
autoend serve
```

`serve` publishes the list of Cursor models it can route (so the dashboard can offer a model picker), then watches Supabase's `runs` table. Start an analysis from Lumen — enter a URL, pick a model — and the daemon claims the run and **streams results as they happen**: screens and edges appear on the map as the agents navigate, and each test flips to pass/fail/warning the moment it settles. Journeys, issues, and insights land in one atomic batch when the Run finishes. (A plain CLI run stays offline and publishes once at the end; only the daemon streams live.)

![The Lumen dashboard rendering a live autoend run](docs/assets/lumen-appmap.png)
...
autoend serve              run as a daemon: watch Supabase for queued runs and stream results live
...
- [x] Results published to Supabase (Lumen dashboard) — verdict, tiers, embedded evidence, generated application map, journeys, and insights. **Known gap:** the dashboard predates the deep pipeline — `defect` findings publish with a mapped severity, but Dispositions and violated expectations aren't rendered there yet; read them from the local `report.json`
- [x] Live streaming daemon (`autoend serve`) — queue runs from the Lumen UI and watch screens, edges, and tests stream in real time; model picker populated from the daemon
