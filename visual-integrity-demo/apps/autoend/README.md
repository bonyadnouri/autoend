...
> Vendored as `@hack-raise/autoend` inside this monorepo (`apps/autoend`) — not published to npm. Run it via the
> workspace, e.g. `pnpm --filter @hack-raise/autoend exec autoend init`, or the local convenience wrapper
> `pnpm test:autoend` (see [scripts/run-autoend.sh](../../scripts/run-autoend.sh)).

```sh
pnpm --filter @hack-raise/autoend exec autoend init    # guided setup — takes a minute
pnpm --filter @hack-raise/autoend exec autoend         # agents test your app, a report opens
```
...
```sh
# from the hack-raise repo root
pnpm install
pnpm --filter @hack-raise/autoend build
pnpm --filter @hack-raise/autoend exec autoend init

# one-time: fetch the headless browser used for replays
pnpm --filter @hack-raise/autoend exec playwright install chromium
```
...
```
$ pnpm --filter @hack-raise/autoend exec autoend
...
```sh
pnpm --filter @hack-raise/autoend exec autoend -e low     # quick pass          ~1-2 min
pnpm --filter @hack-raise/autoend exec autoend -e mid     # everyday runs       ~2-3 min
pnpm --filter @hack-raise/autoend exec autoend -e high    # thorough sweep      ~5 min
pnpm --filter @hack-raise/autoend exec autoend -e xhigh   # deep exploration    ~12 min
pnpm --filter @hack-raise/autoend exec autoend -e ultra   # leave it running    ~35 min
```
...
pnpm --filter @hack-raise/autoend test                                # vitest — includes a real browser replay integration test
pnpm --filter @hack-raise/autoend build                                # tsc -> dist/
pnpm --filter @hack-raise/autoend dev -- http://localhost:3000 -e low --no-open
```
