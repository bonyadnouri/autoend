...
interface Props {
  ...
  /** Overrides the default dismiss warning (e.g. visual findings without a Flow). */
  warningOverride?: string;
}

export function ResolutionActions({ id, action, onResolved, warningOverride }: Props) {
  ...
  return (
    <div>
      ...
      {(warningOverride ?? meta.warning) && (
        <p className="mt-2 text-xs text-slate-500">{warningOverride ?? meta.warning}</p>
      )}
