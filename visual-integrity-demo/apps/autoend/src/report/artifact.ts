...
/**
 * Run artifacts live under .autoend/runs/<runId>/ by default — gitignored,
 * kept until `autoend clean`. `artifactRoot` overrides that root (relative to
 * repoRoot) for parity with hack-raise's HACK_RAISE_ARTIFACT_DIR, e.g. a cloud
 * run honoring AUTOEND_ARTIFACT_DIR.
 */
export function runsDir(repoRoot: string, artifactRoot?: string): string {
  return artifactRoot ? join(repoRoot, artifactRoot) : join(repoRoot, '.autoend', 'runs');
}
...
export async function prepareRunDir(repoRoot: string, runId: string, artifactRoot?: string): Promise<RunDir> {
  const dir = join(runsDir(repoRoot, artifactRoot), runId);
