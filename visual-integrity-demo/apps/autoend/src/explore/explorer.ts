...
export async function explore(opts: ExploreOptions): Promise<ExplorationResult> {
  // On a cloud VM, CURSOR_API_KEY is never set via cloud.envVars (the sidearm
  // refuses to forward CURSOR_*-prefixed vars); instead the VM-side runner
  // maps a Cursor Cloud Secret onto AUTOEND_CURSOR_API_KEY. Prefer a real
  // CURSOR_API_KEY when both are present (e.g. local dev with a stray
  // AUTOEND_CURSOR_API_KEY left in the environment).
  const apiKey = process.env.CURSOR_API_KEY ?? process.env.AUTOEND_CURSOR_API_KEY;
  if (!apiKey) {
    console.warn(
      'exploration skipped: CURSOR_API_KEY not set — run `pnpm --filter @hack-raise/autoend exec autoend init`',
    );
